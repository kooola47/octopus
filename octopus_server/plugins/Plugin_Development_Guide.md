...
def create_incident(title, description="", priority="medium", category="general"):
    ...
    try:
        # Get user parameters (server-side version)
        from routes.user_profile_routes import get_user_parameter
        
        # In server-side plugins, you need to get the username from context
        # This would typically come from the request or session
        username = "current_user"  # Replace with actual username resolution
        
        # Get user's ServiceNow configuration
        api_key = get_user_parameter(username, 'api_credentials', 'servicenow_api_key')
        instance_url = get_user_parameter(username, 'integrations', 'servicenow_instance')
        email = get_user_parameter(username, 'notifications', 'email_address', 'no-reply@company.com')
        ...
        if response.status_code == 201:
            ...
            notify_enabled = get_user_parameter(username, 'notifications', 'notify_on_completion', True)
            if notify_enabled:
...
def get_incident_status(incident_number, username="current_user"):
    """
    Get the status of a ServiceNow incident
    
    Args:
        incident_number: ServiceNow incident number
        username: User to get parameters for
