...
def demo_user_profile(action="get", key=None, value=None):
    """
    Demonstrate secure user profile operations - users can only access their own data
    """
    try:
        # Import appropriate cache manager
        try:
            ...
            import sys
            import os
            sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'octopus_client'))
            from global_cache_manager import get_client_global_cache_manager
            cache_manager = get_client_global_cache_manager()
            location = "client"
        except ImportError:
        ...
        if action == "set" and key and value is not None:
            ...
            cache_manager.set_user_profile_data(key, value)
            result["key"] = key
            result["value"] = value
            result["message"] = f"Set user profile data '{key}' for current user"
            
        elif action == "get" and key:
            # Get user profile data
            profile_data = cache_manager.get_user_profile_data(key)
            result["key"] = key
            result["value"] = profile_data
            result["message"] = f"Retrieved user profile data '{key}' for current user"
            
        elif action == "setting_set" and key and value is not None:
            # Set user setting
            cache_manager.set_user_setting(key, value)
            result["key"] = key
            result["value"] = value
            result["message"] = f"Set user setting '{key}' for current user"
            
        elif action == "setting_get" and key:
            # Get user setting
            setting_value = cache_manager.get_user_setting(key)
            result["key"] = key
            result["value"] = setting_value
            result["message"] = f"Retrieved user setting '{key}' for current user"
            
        elif action == "identity":
            # Show current user identity (client-side only)
            if hasattr(cache_manager, 'get_current_username'):
                username = cache_manager.get_current_username()
                client_id = cache_manager.get_current_client_id()
                result["username"] = username
                result["client_id"] = client_id
                result["message"] = f"Current user identity: {username} (client: {client_id})"
            else:
                result["message"] = "User identity only available on client side"
                
        else:
            result["error"] = f"Unknown action: {action} (available: set, get, setting_set, setting_get, identity)"
...
def run():
    ...
    # Demo user profile (secure access)
    results.append(demo_user_profile("identity"))
    results.append(demo_user_profile("set", "preferences", {"theme": "dark", "language": "en"}))
    results.append(demo_user_profile("get", "preferences"))
    results.append(demo_user_profile("setting_set", "notification_enabled", True))
    results.append(demo_user_profile("setting_get", "notification_enabled"))
    ...
    return {
        "status": "success",
        "message": "Cache demo completed successfully (including secure user profile access)",
        "results": results,
        "type": "cache"
    }
